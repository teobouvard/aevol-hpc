...
for iteration in $(seq 10); do
    for schedule in "guided" "dynamic" "static"; do
                ...
                for grid_size in 32 128 256 512; do
                    for mutation_rate in 0.0001 0.00001 0.000001; do
                        if [[ "$counter" -ge "$already_written" ]]; then
                            printf "Counter : %s\n" "$counter" 1>&2
                            export OMP_SCHEDULE="$schedule" OMP_NUM_THREADS="$n_threads" && $prog -n "$n_steps" -b "$backup_step" -w "$grid_size" -h "$grid_size" -g "$dna_size" -m "$mutation_rate" >>"$output_file"
                        else
