...
counter=0
printf "Skipping %d first steps already written\n" "$already_written"
...
for iteration in $(seq 5); do
    for n_threads in 8 6 4 2 1; do
        for dna_size in 500 5000 50000; do
            for grid_size in 100; do
                grid_width="$grid_size"
                grid_height=$(("$n_threads"*"$grid_size"))
                for mutation_rate in 0.0001 0.00001 0.000001; do
                    if [[ "$counter" -ge "$already_written" ]]; then
                        printf "Counter : %s\n" "$counter" 1>&2
                        export OMP_NUM_THREADS=${n_threads} && $prog -n "$n_steps" -b "$backup_step" -w "$grid_width" -h "$grid_height" -g "$dna_size" -m "$mutation_rate" >> "$output_file"
                    else
