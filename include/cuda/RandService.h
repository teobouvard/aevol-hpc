...
enum Phase { SELECTION = 0, MUTATION = 1, NPHASES = 2 };
...
struct RandService {
  ...
  inline R123_CUDA_DEVICE unsigned int gen_number_max(unsigned int max,
                                                      uint idx, Phase phase) {
    return gen_double(idx, phase) * max;
  }

  R123_CUDA_DEVICE int32_t binomial_random(int32_t nb_drawings, double prob,
                                           uint idx, Phase phase);
  R123_CUDA_DEVICE int random_roulette(double *probability_array,
                                       int nb_elements, uint idx, Phase phase);
};
