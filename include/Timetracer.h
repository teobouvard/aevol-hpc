...
#define CSV_HEADER                                                             \
  "Gen,Resource,Stamp,TimeStamp_Start,TimeStamp_End,Duration,Indiv_id"
...
static void init_tracer(const char *trace_file_name,
                        std::vector<const char *> stamps) {
...
static void timestamp_start() { starts[GET_RESOURCE].push_back(GET_TIME); }
...
static void write_traces(int generation) {
  for (int res = 0; res < nb_resources; ++res) {
    for (int stamp = 0; stamp < stamp_int[res].size(); ++stamp) {
      trace_file << generation << "," << (res + 1) << ","
                 << stamp_name[stamp_int[res][stamp]] << ","
                 << starts[res][stamp] << "," << ends[res][stamp] << ","
                 << ends[res][stamp] - starts[res][stamp] << ","
                 << indiv_id[res][stamp] << std::endl;
    }
  }
  trace_file.flush();
  set_traces();
...
#define INIT_TRACER(file_name, ...)                                            \
  time_tracer::init_tracer(file_name, __VA_ARGS__);
