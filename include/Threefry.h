...
public:
  ...
  Threefry(int X, int Y, unsigned int seed)
  ...
  class Gen {
    ...
    Gen(Threefry *parent, size_t idx, Phase phase) : parent_(parent) {
  ...
  public:
    ...
    unsigned int random(unsigned int max) { return random() * max; }

    int32_t roulette_random(double *probs, int32_t nb_elts,
                            bool verbose = false);

    int32_t
    binomial_random(int32_t nb,
                    double prob); // Binomial drawing of parameters (nb, prob)
  };
  ...
  Gen gen(size_t idx, Phase phase) { return std::move(Gen(this, idx, phase)); }
