// ***************************************************************************************************************
//
//          Mini-Aevol is a reduced version of Aevol -- An in silico
//          experimental evolution platform
//
// ***************************************************************************************************************
//
// Copyright: See the AUTHORS file provided with the package or
// <https://gitlab.inria.fr/rouzaudc/mini-aevol> Web:
// https://gitlab.inria.fr/rouzaudc/mini-aevol E-mail: See
// <jonathan.rouzaud-cornabas@inria.fr> Original Authors : Jonathan
// Rouzaud-Cornabas
//
...
/**
 * Constructor to generate a random organism (i.e. an organism with a random
 * DNA)
 *
 ...
 */
void Organism::save(gzFile backup_file) const { dna_->save(backup_file); }
...
/**
 * Reset the stats variable (used when an organism is a perfect clone of its
 * parent, it means no mutation)
 */
    ...
    if (terminator_found) {
      ...
      if (rna_length > 0) {
        ...
        rnas[glob_rna_idx] = new RNA(
            prom_pos, rna_end, 1.0 - std::fabs(((float)prom_pair.second)) / 5.0,
            rna_length);
      }
    }
  }
...
void Organism::compute_protein() {
  ...
  for (int rna_idx = 0; rna_idx < rna_count_; rna_idx++) {
    ...
    for (int protein_idx = 0; protein_idx < rna->start_prot.size();
         protein_idx++) {
      ...
      while (rna->length - transcription_length >= CODON_SIZE) {
        if (dna_->protein_stop(current_position)) {
          ...
          if (protein_start + SD_TO_START < protein_end) {
            ...
            prot_length =
                (length() + protein_end) - (protein_start + SD_TO_START);
          }

          if (prot_length >
              CODON_SIZE) { // it has at least 2 codons, among them a STOP
            int glob_prot_idx = protein_count_;
            protein_count_ += 1;

            proteins[glob_prot_idx] =
                new Protein(protein_start, protein_end, prot_length, rna->e);
...
void Organism::translate_protein() {
  for (int protein_idx = 0; protein_idx < protein_count_; protein_idx++) {
    ...
    if (protein->is_init_) {
      ...
      int nb_codon =
          (protein->protein_length / 3) - 1; // Do not count the STOP codon
       ...
       * It looks like black magic (again) but the ide of the implementation can
       * be found hear:
      ...
      bool bin_m = false; // Initializing to false will yield a conservation of
                          // the high weight bit
      bool bin_w = false; // when applying the XOR operator for the Gray to
                          // standard conversion
      bool bin_h = false;
      ...
      for (int i = 0; i < nb_codon; i++) {
        ...
        case CODON_M0: {
          ...
          bin_m ^= false; // as bin_m was initialized to false, the XOR will
                          // have no effect on the high weight bit

          // A lower-than-the-previous-lowest weight bit was found, make a left
          // bitwise shift
          //~ M <<= 1;
          M *= 2;
        ...
        case CODON_M1: {
          ...
          // Convert Gray code to "standard" binary code
          bin_m ^= true; // as bin_m was initialized to false, the XOR will have
                         // no effect on the high weight bit

          // A lower-than-the-previous-lowest bit was found, make a left bitwise
          // shift
          //~ M <<= 1;
          M *= 2;
        ...
        case CODON_W0: {
          ...
          // Convert Gray code to "standard" binary code
          bin_w ^= false; // as bin_m was initialized to false, the XOR will
                          // have no effect on the high weight bit
        ...
        case CODON_W1: {
          ...
          // Convert Gray code to "standard" binary code
          bin_w ^= true; // as bin_m was initialized to false, the XOR will have
                         // no effect on the high weight bit
        ...
        case CODON_H0:
        case CODON_START: // Start codon codes for the same amino-acid as H0
                          // codon
        {
          // H codon found
          nb_h++;
          ...
          // Convert Gray code to "standard" binary code
          bin_h ^= false; // as bin_m was initialized to false, the XOR will
                          // have no effect on the high weight bit
        ...
        case CODON_H1: {
          ...
          // Convert Gray code to "standard" binary code
          bin_h ^= true; // as bin_m was initialized to false, the XOR will have
                         // no effect on the high weight bit
      ...
      //  ----------------------------------------------------------------------------------
      //  2) Normalize M, W and H values in [0;1] according to number of codons
      //  of each kind
      //  ----------------------------------------------------------------------------------
      ...
      //  ------------------------------------------------------------------------------------
      //  3) Normalize M, W and H values according to the allowed ranges
      //  (defined in macros.h)
      //  ------------------------------------------------------------------------------------
      // x_min <= M <= x_max
      // w_min <= W <= w_max
      ...
      if (nb_m == 0 || nb_w == 0 || nb_h == 0 || protein->w == 0.0 ||
          protein->h == 0.0) {
...
void Organism::compute_phenotype() {
  ...
  for (int protein_idx = 0; protein_idx < protein_count_; protein_idx++) {
    ...
    if (protein->is_init_ && protein->is_functional) {
      ...
      if (ix0 < 0)
        ix0 = 0;
      else if (ix0 > (FUZZY_SAMPLING - 1))
        ix0 = FUZZY_SAMPLING - 1;
      if (ix1 < 0)
        ix1 = 0;
      else if (ix1 > (FUZZY_SAMPLING - 1))
        ix1 = FUZZY_SAMPLING - 1;
      if (ix2 < 0)
        ix2 = 0;
      else if (ix2 > (FUZZY_SAMPLING - 1))
        ix2 = FUZZY_SAMPLING - 1;
...
void Organism::compute_fitness(const double *target) {
  ...
  for (int fuzzy_idx = 0; fuzzy_idx < FUZZY_SAMPLING - 1; fuzzy_idx++) {
    ...
    metaerror +=
        (std::fabs(delta[fuzzy_idx]) + std::fabs(delta[fuzzy_idx + 1])) /
...
void Organism::remove_promoters_around(int32_t pos_1, int32_t pos_2) {
  if (mod(pos_1 - pos_2, dna_->length()) >= PROM_SIZE) {
    remove_promoters_starting_between(
        mod(pos_1 - PROM_SIZE + 1, dna_->length()), pos_2);
  } else {
    remove_all_promoters();
  }
}
...
void Organism::look_for_new_promoters_around(int32_t pos) {
  if (dna_->length() >= PROM_SIZE) {
    look_for_new_promoters_starting_between(
        mod(pos - PROM_SIZE + 1, dna_->length()), pos);
  }
}

void Organism::remove_all_promoters() { promoters_.clear(); }
...
void Organism::remove_promoters_starting_between(int32_t pos_1, int32_t pos_2) {
  if (pos_1 > pos_2) {
    ...
    promoters_.erase(promoters_.lower_bound(pos_1),
                     promoters_.upper_bound(pos_2 - 1));
  }
}
...
void Organism::add_new_promoter(int32_t position, int8_t error) {
  // TODO: Insertion should not always occur, especially if promoter become
  // better or worse ? Promoters are deleted anyway if victim of mutation. the
  // IF stays unnecessary
  if (promoters_.find(position) == promoters_.end())
    promoters_[position] = error;
}

void Organism::look_for_new_promoters_starting_between(int32_t pos_1,
                                                       int32_t pos_2) {
  ...
  for (int32_t i = pos_1; i < pos_2; i++) {
    ...
    if (dist <= PROM_MAX_DIFF) { // dist takes the hamming distance of the
...
void Organism::look_for_new_promoters_starting_after(int32_t pos) {
  for (int32_t i = pos; i < dna_->length(); i++) {
    ...
    if (dist <= 4) { // dist takes the hamming distance of the sequence from the
                     // consensus
      add_new_promoter(i, dist);
    }
  }
