...
// <https://gitlab.inria.fr/rouzaudc/mini-aevol>
...
void print_help(char *prog_path) {
  ...
  printf("*                                                                    "
         "        *\n");
  printf("* This is a mini-version with a cutdown biological model and a lot "
         "of        *\n");
  ...
  printf(
      "  -w, --width WIDTH_SIZE\tWidth of the population grid is WIDTH_SIZE\n");
  printf("  -h, --height HEIGHT_SIZE\tHeight of the population grid is "
         "HEIGHT_SIZE\n");
  printf("  -m, --mutation_rate MUTATION_RATE\tMutation rate is set to "
         "MUTATION_RATE\n");
  printf("  -g, --genome_size GENOME_SIZE\tGenome at the initial genome is "
         "GENOME_SIZE bps\n");
  printf("  -b, --backup_step BACKUP_STEP\tDo a simulation backup/checkpoint "
         "every BACKUP_STEP\n");
  printf("  -r, --resume RESUME_STEP\tResume the simulation from the "
         "RESUME_STEP generations\n");
  printf(
      "  -s, --seed SEED\tChange the seed for the pseudo random generator\n");
}
...
int main(int argc, char *argv[]) {
  ...
  if (resume == -1) {
    exp_manager = new ExpManager(height, width, seed, mutation_rate,
                                 genome_size, backup_step);
  } else {
    printf("Resuming...\n");
...
#ifdef USE_CUDA
  // Not very clean but the goal is to re-use the initialization on the host to
