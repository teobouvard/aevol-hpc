...
/**
 * Create the data structure and open the file for the statistics of a
 * simulation
 *
 * @param generation : Create statistics beginning from this generation (or
 * resuming from this generation)
 * @param best_or_not : Statistics for the best organisms or mean of all the
 * organisms
 */
Stats::Stats(int generation, bool best_or_not) {
  ...
  if (generation_ == 0) {
    ...
    if (is_indiv_) {
      ...
      statfile_best_.open("stats/stats_simd_best.csv.tmp",
                          std::ofstream::trunc);
    } else {
      tmp_mean.open("stats/stats_simd_mean.csv", std::ifstream::in);
      statfile_mean_.open("stats/stats_simd_mean.csv.tmp",
                          std::ofstream::trunc);
    }
    ...
    tmp_best.close();
    tmp_mean.close();
    is_indiv_ ? std::remove("stats/stats_simd_best.csv.tmp")
 ...
 */
void Stats::compute_average(const std::shared_ptr<Organism> *population,
                            int population_size) {
  is_indiv_ = false;
...
/**
 * Write the statistics of the mean of the population to the related statistics
 * file
 */
void Stats::write_average(const std::shared_ptr<Organism> *population,
                          int population_size) {
  if (!is_indiv_ && !is_computed_)
  ...
  if (!is_indiv_ && is_computed_) {
    // Write average stats
    statfile_mean_ << generation_ << "," << mean_fitness_ << ","
                   << mean_metabolic_error_ << "," << mean_amount_of_dna_ << ","
