...
int Dna::length() const { return seq_.size(); }
...
void Dna::set(int pos, char c) { seq_[pos] = c; }
...
/**
 * Insert a sequence of a given length at a given position into the DNA of the
 * Organism
 *
...
void Dna::do_duplication(int pos_1, int pos_2, int pos_3) {
  ...
  if (pos_1 < pos_2) {
    ...
    //
    std::vector<char> seq_dupl =
        std::vector<char>(seq_.begin() + pos_1, seq_.end());
...
int Dna::promoter_at(int pos) {
  ...
  for (int motif_id = 0; motif_id < PROM_SIZE; motif_id++) {
    ...
    prom_dist[motif_id] = PROM_SEQ[motif_id] == seq_[search_pos] ? 0 : 1;
  }

  // Computing if a promoter exists at that position
  int dist_lead = prom_dist[0] + prom_dist[1] + prom_dist[2] + prom_dist[3] +
                  prom_dist[4] + prom_dist[5] + prom_dist[6] + prom_dist[7] +
                  prom_dist[8] + prom_dist[9] + prom_dist[10] + prom_dist[11] +
                  prom_dist[12] + prom_dist[13] + prom_dist[14] +
                  prom_dist[15] + prom_dist[16] + prom_dist[17] +
                  prom_dist[18] + prom_dist[19] + prom_dist[20] + prom_dist[21];
...
int Dna::terminator_at(int pos) {
  ...
  int dist_term_lead =
      term_dist[0] + term_dist[1] + term_dist[2] + term_dist[3];
