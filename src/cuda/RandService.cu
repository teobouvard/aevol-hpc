...
__device__ int RandService::random_roulette(double *probability_array,
                                            int nb_elements, uint idx,
                                            Phase phase) {
...
static __constant__ double cof[6] = {
    76.18009172947146,  -86.50532032941677,    24.01409824083091,
    -1.231739572450155, 0.1208650973866179e-2, -0.5395239384953e-5};

// Returns the value ln[gamma(X)] for X.
// The gamma function is defined by the integral  gamma(z) = int(0, +inf,
// t^(z-1).e^(-t)dt). When the argument z is an integer, the gamma function is
// just the familiar factorial function, but offset by one, n! = gamma(n + 1).
...
__device__ int32_t RandService::binomial_random(int32_t nb_drawings,
                                                double prob, uint idx,
                                                Phase phase) {
  int32_t nb_success;
  ...
  // This can require up to 25 calls to the uniform random.
  {
    ...
    for (int32_t j = 1; j <= nb_drawings; j++) {
      if (gen_double(idx, phase) < p)
        nb_success++;
    }
  } else if (mean < 1.0)
  // If fewer than one event is expected out of 25 or more trials,
  // then the distribution is quite accurately Poisson. Use direct Poisson
  // method.
  {
    double g = exp(-mean);
    double t = 1.0;
  ...
  // Use the rejection method.
  {
    ...
    do {
      ...
      t = 1.2 * sq * (1.0 + y * y) *
        ...
        gen_double(idx, phase) >
        t); // Reject. This happens about 1.5 times per deviate, on average.
  ...
  if (p != prob)
    nb_success = nb_drawings - nb_success;
