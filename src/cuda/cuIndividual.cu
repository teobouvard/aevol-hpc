...
__device__ void cuIndividual::compute_rna(uint rna_idx) const {
  ...
  uint idx_term =
      find_smallest_greater(start_transcript, terminators, nb_terminator);
  uint term_position = terminators[idx_term];
  uint transcript_length =
      get_distance(start_transcript, term_position) + TERM_SIZE;
...
__device__ void cuIndividual::prepare_gene(uint rna_idx) const {
  ...
  uint first_next_ps =
      find_smallest_greater(rna.start_transcription, list_ps, nb_ps);
  ...
  for (int i = 0; i < rna.nb_gene; ++i) {
    ...
    rna.list_gene[i].length_limit =
        get_distance(rna.start_transcription, start);
...
__device__ void cuIndividual::gather_genes() {
  ...
  for (int idx_rna = 0; idx_rna < nb_rnas; ++idx_rna) {
    ...
    for (int i = 0; i < rna.nb_gene; ++i) {
      ...
      // limit is difference between transcription_length and distance start_rna
      // -> start_gen (computed in `prepare_gene`)
      list_gene[insert_idx].length_limit =
          rna.transcription_length - list_gene[insert_idx].length_limit;
      insert_idx++;
    }
  }
}
...
__device__ void cuIndividual::translate_gene(uint gene_idx) const {
  ...
  if (distance) {
    ...
    new_protein.concentration =
        (float)gene.concentration / (float)(PROM_MAX_DIFF + 1);
...
__device__ void cuIndividual::compute_phenotype() {
  ...
  __shared__ double phenotype_activ_inhib[2]
                                         [FUZZY_SAMPLING]; // { activ_phenotype,
                                                           // inhib_phenotype }
  ...
  for (int fuzzy_idx = idx; fuzzy_idx < FUZZY_SAMPLING; fuzzy_idx += rr_width) {
    phenotype_activ_inhib[0][fuzzy_idx] =
        min(phenotype_activ_inhib[0][fuzzy_idx], 1.0);
    phenotype_activ_inhib[1][fuzzy_idx] =
        max(phenotype_activ_inhib[1][fuzzy_idx], -1.0);
    // phenotype = active_phenotype + inhib_phenotype (inhib_phenotype < 0)
    phenotype[fuzzy_idx] = phenotype_activ_inhib[0][fuzzy_idx] +
                           phenotype_activ_inhib[1][fuzzy_idx];
  ...
  if (threadIdx.x == 0) {
    ...
    for (int i = 0; i < local_nb_gene; ++i) {
      ...
      printf("\t%d: concentration: %d, limit: %d\n", list_gene[i].start,
             list_gene[i].concentration, list_gene[i].length_limit);
    }
