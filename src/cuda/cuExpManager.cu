...
cuExpManager::cuExpManager(const ExpManager *cpu_exp) {
  ...
  for (int i = 0; i < nb_indivs_; ++i) {
    ...
    memcpy(host_individuals_[i], org->dna_->seq_.data(),
           genome_length_ * sizeof(char));
  }
...
void cuExpManager::run_a_step() {
  ...
  selection<<<grid_dim, bloc_dim>>>(grid_height_, grid_width_,
                                    device_individuals_, rand_service_,
                                    reproducers_);

  // Reproduction
  reproduction<<<nb_indivs_, threads_per_block>>>(
      nb_indivs_, device_individuals_, reproducers_, all_parent_genome_);

  // Mutation
  auto grid_dim_1d = ceil((float)nb_indivs_ / (float)threads_per_block);
  do_mutation<<<grid_dim_1d, threads_per_block>>>(
      nb_indivs_, device_individuals_, mutation_rate_, rand_service_);

  // Evaluation
  evaluate_population<<<nb_indivs_, threads_per_block>>>(
      nb_indivs_, device_individuals_, device_target_);

  // Swap genome information
  swap_parent_child_genome<<<grid_dim_1d, threads_per_block>>>(
      nb_indivs_, device_individuals_, all_parent_genome_);
...
void cuExpManager::save(int t) const {
  ...
  for (int indiv_id = 0; indiv_id < nb_indivs_; indiv_id++) {
    ...
    gzwrite(exp_backup_file, host_individuals_[indiv_id],
            genome_length_ * sizeof(char));
  }
...
void cuExpManager::transfer_to_device() {
  ...
  checkCuda(
      cudaMalloc(&(device_individuals_), nb_indivs_ * sizeof(cuIndividual)));
  ...
  checkCuda(cudaMalloc(&(all_child_genome_),
                       all_genomes_size_w_phantom * sizeof(char)));
  checkCuda(cudaMalloc(&(all_parent_genome_),
                       all_genomes_size_w_phantom * sizeof(char)));
  ...
  for (int i = 0; i < nb_indivs_; ++i) {
    ...
    checkCuda(cudaMemcpy(indiv_genome_pointer, host_individuals_[i],
                         genome_length_, cudaMemcpyHostToDevice));
    checkCuda(cudaMemcpy(indiv_genome_phantom_pointer, host_individuals_[i],
                         PROM_SIZE, cudaMemcpyHostToDevice));
  }

  init_device_population<<<1, 1>>>(
      nb_indivs_, genome_length_, device_individuals_, all_child_genome_,
  ...
  checkCuda(cudaMemcpy(device_target_, target_, FUZZY_SAMPLING * sizeof(double),
  ...
  RandService tmp;
  checkCuda(
      cudaMalloc(&(tmp.rng_counters), nb_counter_ * sizeof(ctr_value_type)));
  checkCuda(cudaMemcpy(tmp.rng_counters, counters_,
                       nb_counter_ * sizeof(ctr_value_type),
                       cudaMemcpyHostToDevice));
  tmp.seed = {{0, seed_}};
  ...
  checkCuda(cudaMemcpy(rand_service_, &tmp, sizeof(RandService),
                       cudaMemcpyHostToDevice));
...
void cuExpManager::transfer_to_host() const {
  for (int i = 0; i < nb_indivs_; ++i) {
    ...
    checkCuda(cudaMemcpy(host_individuals_[i], indiv_genome_pointer,
                         genome_length_, cudaMemcpyDeviceToHost));
  }

  RandService tmp;
  checkCuda(cudaMemcpy(&tmp, rand_service_, sizeof(RandService),
                       cudaMemcpyDeviceToHost));
  checkCuda(cudaMemcpy(counters_, tmp.rng_counters,
                       nb_counter_ * sizeof(ctr_value_type),
                       cudaMemcpyDeviceToHost));
}
...
void cuExpManager::device_data_destructor() {
  ...
  checkCuda(cudaMemcpy(&tmp_rand, rand_service_, sizeof(RandService),
                       cudaMemcpyDeviceToHost));
  ...
  cuIndividual tmp;
  checkCuda(cudaMemcpy(&tmp, device_individuals_, sizeof(cuIndividual),
                       cudaMemcpyDeviceToHost));
...
__global__ void check_result(uint nb_indivs, cuIndividual *individuals) {
...
__global__ void evaluate_population(uint nb_indivs, cuIndividual *individuals,
                                    const double *target) {
...
__global__ void do_mutation(uint nb_indivs, cuIndividual *individuals,
                            double mutation_rate, RandService *rand_service) {
  ...
  auto nb_switch = rand_service->binomial_random(indiv.size, mutation_rate,
                                                 indiv_idx, MUTATION);

  for (int i = 0; i < nb_switch; ++i) {
    auto position =
        rand_service->gen_number_max(indiv.size, indiv_idx, MUTATION);
    if (indiv.genome[position] == '0')
      indiv.genome[position] = '1';
    else
      indiv.genome[position] = '0';
...
__global__ void reproduction(uint nb_indivs, cuIndividual *individuals,
                             const int *reproducers,
                             const char *all_parent_genome) {
  ...
  if (threadIdx.x == 0) {
    ...
    size = child.size;
    auto offset = reproducers[indiv_idx] *
                  (child.size + PROM_SIZE); // Do not forget phantom space
    parent_genome = all_parent_genome + offset;
    child_genome = child.genome;
  }
  __syncthreads();

  for (int position = idx; position < size + PROM_SIZE;
       position += rr_width) { // Do not forget phantom space
    child_genome[position] = parent_genome[position];
  }
}

__global__ void selection(uint grid_height, uint grid_width,
                          const cuIndividual *individuals,
                          RandService *rand_service, int *next_reproducers) {
  ...
  auto selected_cell = rand_service->random_roulette(
      local_fit_array, NEIGHBORHOOD_SIZE, grid_idx, SELECTION);
...
__global__ void swap_parent_child_genome(uint nb_indivs,
                                         cuIndividual *individuals,
                                         char *all_parent_genome) {
  // One thread per individual
  auto indiv_idx = threadIdx.x + blockIdx.x * blockDim.x;
  if (indiv_idx >= nb_indivs)
    return;
  ...
  auto &indiv = individuals[indiv_idx];
  auto offset =
      indiv_idx * (indiv.size + PROM_SIZE); // Do not forget phantom space
  indiv.genome = all_parent_genome + offset;
}

// Interface Host | Device

__global__ void clean_population_metadata(uint nb_indivs,
                                          cuIndividual *individuals) {
...
__global__ void check_target(double *target) {
  for (int i = 0; i < FUZZY_SAMPLING; ++i) {
    if (target[i] == 0.0) {
      printf("0|");
    } else {
...
__global__ void check_rng(RandService *rand_service) {
  auto idx = threadIdx.x + blockIdx.x * blockDim.x;
  auto number = rand_service->generator(
      {{0, rand_service->rng_counters[rand_service->phase_size]}},
      rand_service->seed);
  if (idx == 0) {
    printf("seed: %lu, counter[nb_indivs]: %lu, test: %lu\n",
           rand_service->seed[1],
           rand_service->rng_counters[rand_service->phase_size], number[0]);
  }
}

__global__ void init_device_population(int nb_indivs, int genome_length,
                                       cuIndividual *all_individuals,
                                       char *all_genomes,
                                       uint8_t *all_promoters,
                                       uint *all_terminators,
                                       uint *all_prot_start, cuRNA *all_rnas) {
  auto idx = threadIdx.x + blockIdx.x * blockDim.x;
