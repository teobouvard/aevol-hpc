...
void Threefry::save(gzFile backup_file) const {
  ...
  gzwrite(backup_file, counters_.data(),
          counters_.size() * sizeof(counters_[0]));
}
...
Threefry::Threefry(int X, int Y, gzFile backup_file)
  ...
  counters_ = std::vector<crt_value_type>(tmp_counters,
                                          tmp_counters + counters_.size());
}

int32_t Threefry::Gen::roulette_random(double *probs, int32_t nb_elts,
                                       bool verbose) {
