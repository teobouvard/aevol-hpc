 ...
 * @param mutation_rate : Mutation rates for all the organism during the
 * simulation
 * @param init_length_dna : Size of the randomly generated DNA at the
 * initialization of the simulation
 * @param selection_pressure : Selection pressure used during the selection
 * process
 * @param backup_step : How much often checkpoint must be done
 */
ExpManager::ExpManager(int grid_height, int grid_width, int seed,
                       double mutation_rate, int init_length_dna,
                       int backup_step)
  ...
  for (int i = 0; i < FUZZY_SAMPLING - 1; i++) {
    ...
    geometric_area += ((fabs(target[i]) + fabs(target[i + 1])) /
  ...
  while (r_compare >= 0) {
    auto random_organism = std::make_shared<Organism>(
        init_length_dna, rng_->gen(0, Threefry::MUTATION));
...
/**
 * Constructor to resume/restore a simulation from a given backup/checkpoint
 * file
 *
...
void ExpManager::load(int t) {
  ...
  // No need to save/load this field from the backup because it will be set at
  // selection()
  next_generation_reproducer_ = new int[nb_indivs_]();
  ...
  rng_ = std::move(
      std::make_unique<Threefry>(grid_width_, grid_height_, exp_backup_file));
  seed_ = rng_->get_seed();
...
/**
 * Selection process: for a given cell in the grid of the population, compute
 * which organism win the computation
 *
  ...
  for (int8_t i = -1; i < NEIGHBORHOOD_WIDTH - 1; i++) {
    for (int8_t j = -1; j < NEIGHBORHOOD_HEIGHT - 1; j++) {
      ...
      local_fit_array[count] =
          prev_internal_organisms_[cur_x * grid_width_ + cur_y]->fitness;
...
void ExpManager::prepare_mutation(int indiv_id) const {
  auto *rng =
      new Threefry::Gen(std::move(rng_->gen(indiv_id, Threefry::MUTATION)));
  const shared_ptr<Organism> &parent =
      prev_internal_organisms_[next_generation_reproducer_[indiv_id]];
  dna_mutator_array_[indiv_id] =
      new DnaMutator(rng, parent->length(), mutation_rate_);
